...
ROSTROS_TXT = os.path.join(os.path.dirname(__file__), 'rostros_"+anio+".txt')
...
# Procesar solo la imagen anio.jpg
filename = anio+".jpg"
...
for idx, (x, y, w, h) in enumerate(faces, start=1):
    ...
    nombre = nombres_caras.get(anio, {}).get(idx, '')
...
# Actualizar solo la entrada para anio.jpg
...
# Mostrar estadísticas específicas para anio
if anio+".jpg" in faces_numbered:
    caras_2000 = faces_numbered[anio+".jpg"]
    caras_con_nombre_2000 = sum(1 for cara in caras_2000 if cara['nombre'])
    print(f"\nEstadísticas para {anio}:")
...
with open(ROSTROS_TXT, 'w', encoding='utf-8') as f:
    f.write("INFORMACIÓN DE ROSTROS DETECTADOS - ",anio,"\n")
    f.write("=" * 50 + "\n\n")
...
if img_with_rectangles is not None:
    ...
    output_filename = anio+" - numerada.jpg"
